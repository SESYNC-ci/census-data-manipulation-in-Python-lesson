...
cbp2 = cbp2.assign(FIPS=lambda x: x['FIPSTATE']+x['FIPSCTY'],
...
Alternatively, we can use other methods to select using match patterns. For instance, select all columns starting with N using the filter command

```{python select, handout = 0}
cbp4 = cbp.filter(items=['FIPS','NAICS'],axis=1) 
cbp4.filter(regex='^N',axis=1) 
...
test=cbp.filter(regex='^N',axis=1) | cbp.filter(items=['FIPS','NAICS'],axis=1)

#cbp.filter(items=['FIPS','NAICS'],axis=1)
#cbp4.head()

#cbp[['FIPS','NAICS'] | ,axis=1)
#cbp4.head()
